...
class EventSpider(scrapy.Spider):
    name = 'event_htn'
    mcpcty = 'HTN'
    allowed_domains = ['hungthinhincons.com.vn'] 
    ...
    async def start(self):
        urls = [
            ('https://hungthinhincons.com.vn/investor-relation/Thong-tin-co-dong.html', self.parse_generic),
            ('https://hungthinhincons.com.vn/investor-relation/Bao-cao-tai-chinh.html', self.parse_generic),
        ]
    ...
    async def parse_generic(self, response):
        ...
        rows = response.css('table.tablecodong tr:not(:first-child)')

        for row in rows:
            title = row.css('td:nth-child(1) a::text').get()
            relative_url = row.css('td:nth-child(1) a::attr(href)').get()
            date_str = row.css('td:nth-child(2)::text').get()
