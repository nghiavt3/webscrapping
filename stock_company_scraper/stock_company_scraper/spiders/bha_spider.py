...
class EventSpider(scrapy.Spider):
    name = 'event_bha'
    mcpcty = 'BHA' 
    allowed_domains = ['thuydienbacha.vn'] 
    ...
    async def start(self):
        urls = [
            ('https://thuydienbacha.vn/category/quan-he-co-dong/', self.parse_generic),
    ...
    async def parse_generic(self, response):
        ...
        articles = response.css('div.post-item.cate, div.post_home_row4')
        
        for art in articles:
            link_node = art.css('a')   
            title = link_node.attrib.get('title') or art.css('h4::text').get()
            raw_date = art.css('.post-meta::text').get() or art.css('p ::text').getall()
