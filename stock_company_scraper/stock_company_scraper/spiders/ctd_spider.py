...
class EventSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for item in news_items:
            # Lấy URL
            url = item.css('article a::attr(href)').get()

            # Lấy Tiêu đề
            title = item.css('article h3.elementor-icon-box-title a::text').get().strip()

            # Lấy Ngày tháng
            date = item.css('article span.elementor-post-info__item--type-date::text').get().strip()
