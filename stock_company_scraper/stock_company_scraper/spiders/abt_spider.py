...
class EventSpider(scrapy.Spider):
    name = 'event_abt'
    mcpcty = 'ABT' 
    allowed_domains = ['aquatexbentre.com'] 
    ...
    def start_requests(self):
        urls = [
            ('https://aquatexbentre.com/cong/', self.parse_generic),
    ...
    def parse_generic(self, response):
        ...
        items = response.css('.ux-menu-link.menu-item')
        
        for item in items:         
            raw_text = item.css('.ux-menu-link__text::text').get()
            if not raw_text:
                continue
            # Xử lý chuỗi: "29/12/2025 10:00 NQ HĐQT..."
            # Tách chuỗi bằng khoảng trắng
            parts = raw_text.strip().split()
            # Phần tử đầu tiên là ngày: 29/12/2025
            raw_date = parts[0]
            title = " ".join(parts[2:]) if ":" in parts[1] else " ".join(parts[1:])
