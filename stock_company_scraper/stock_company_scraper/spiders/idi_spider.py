...
class EventSpider(scrapy.Spider):
    name = 'event_idi'
    mcpcty = 'IDI' 
    allowed_domains = ['idiseafood.com'] 
    ...
    def start_requests(self):
        urls = [
            ('https://www.idiseafood.com/vn/quan-he-co-dong.html', self.parse_generic),
    ...
    def parse_generic(self, response):
        ...
        for item in response.css('li.itemshare'):           
            title = item.css('div.hcol h3 a::text').get().strip()
            date = item.css('div.date span::text').get()
            link = item.css('div.mmdown a::attr(href)').get()
            detail_link = item.css('div.hcol h3 a::attr(href)').get()
            if not title:
                continue
