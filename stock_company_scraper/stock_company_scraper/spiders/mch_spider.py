...
class EventSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for row in rows:
            # 2. Trích xuất Ngày
            # Ngày nằm trong thẻ <td> đầu tiên
            date = row.css('td:nth-child(1)::text').get()
            
            # 3. Trích xuất Nội dung
            # Nội dung nằm trong thẻ <a> bên trong td thứ hai
            title = row.css('td:nth-child(2) .wrapper .label a::text').get()
            
            # 4. Trích xuất Liên kết PDF (Link tải về)
            # Link nằm trong thuộc tính href của thẻ <a> đó
            pdf_link = row.css('td:nth-child(2) .wrapper .label a::attr(href)').get()
            
            # Làm sạch dữ liệu (loại bỏ khoảng trắng thừa, ký tự xuống dòng...)
            if date:
                date = date.strip()
            if title:
                # Loại bỏ các khoảng trắng và ký tự xuống dòng ở đầu/cuối
                title = title.strip()
