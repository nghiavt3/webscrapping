...
class EventSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [
            ('http://nuinho.vn/quan-he-co-dong.html', self.parse_generic),
    ...
    def parse_generic(self, response):
        ...
        posts = response.css('#right .name_spc2')
        
        for post in posts:      
            title = post.css('a::text').get()
            raw_date_text = post.xpath('following-sibling::div[1][contains(@class, "datetime")]/text()').get()
            clean_date = raw_date_text.replace('Ngày đăng:', '').strip()
