...
class EventSpider(scrapy.Spider):
    ...
    async def start(self):
        urls = [
            ('https://bmsc.com.vn/tin-co-dong/', self.parse),
            ('https://bmsc.com.vn/download/Bao-cao-tai-chinh/', self.parse_bctc),
        ]
    ...
    def parse_bctc(self, response):
        ...
        # 2. Duyệt qua danh sách tin tức
        rows = response.css('div.list-content')
        
        for row in rows:
            # Trích xuất dữ liệu
            title_node = row.css('div.col-md-10 a')
            title = title_node.css('::text').get()
            link = title_node.css('::attr(href)').get()
            date= row.css('div.col-md-6::text').get().strip()
            # Trích xuất ngày đăng (xử lý getall() để nối chuỗi văn bản)
            
            
            # Làm sạch dữ liệu
            clean_title = title.strip() if title else ""
            iso_date = convert_date_to_iso86012(date)
            full_link = response.urljoin(link)
...
def convert_date_to_iso8601(vietnam_date_str):
    ...
    try:
        ...
        try:
            ...
            return clean_str
def convert_date_to_iso86012(vietnam_date_str):
    """
    Xử lý: '29/12/2025 02:30:15' -> '2025-12-29'
    Hỗ trợ cả trường hợp chỉ có ngày DD/MM/YYYY
    """
    if not vietnam_date_str:
        return None
    ...
    # Thử định dạng đầy đủ (Ngày Giờ AM/PM)
    try:
        date_object = datetime.strptime(clean_str, '%d/%m/%Y %H:%M')
        return date_object.strftime(output_format)
    except ValueError:
        # Thử định dạng chỉ có ngày
        try:
