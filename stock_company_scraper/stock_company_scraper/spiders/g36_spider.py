...
class EventSpider(scrapy.Spider):
    ...
    async def start(self):
        yield scrapy.Request(
            url='https://36corp.com/quan-he-co-dong/cong-bo-thong-tin/',
            callback=self.parse,
        )
        yield scrapy.Request(
            url='https://36corp.com/quan-he-co-dong/bao-cao-tai-chinh/',
    ...
    def parse(self, response):
        ...
        for post in posts:
            # Trích xuất dữ liệu
            date_raw = post.css('.entry-meta a time.entry-date::text').get()
    ...
    def parse_bctc(self, response):
        ...
        # 2. Lấy danh sách bài viết (article)
        container = response.css('div.dropdowncontent-content')
        
        for article in container.css('article.post'):
            # Trích xuất dữ liệu
            date_raw = article.css('time.entry-date::text').get()
            title_raw = article.attrib.get('data-name')
            url_raw = article.css('h3.entry-title a::attr(href)').get()
