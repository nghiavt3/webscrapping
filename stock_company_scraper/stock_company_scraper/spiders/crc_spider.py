...
class EventSpider(scrapy.Spider):
    ...
    async def start(self):
        urls = [
            ('https://createcapital.vn/quan-he-co-dong-crc.htm', self.parse),
            ('https://createcapital.vn/tin-tuc.htm', self.parse_tintuc),
        ]
    ...
    def parse_tintuc(self, response):
        ...
        # 2. Duyệt qua các item tin tức
        items = response.css('div.col4.item')
        for item in items:
            
            url = item.css('a::attr(href)').get()
            title_raw = item.css('h3.dot3::text').get()
