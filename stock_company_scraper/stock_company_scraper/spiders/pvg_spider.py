...
class EventSpider(scrapy.Spider):
    name = 'event_pvg'
    mcpcty = 'PVG'
    allowed_domains = ['pvgaslpg.com.vn'] 
    start_urls = [
        'https://pvgaslpg.com.vn/vi/quan-he-co-dong/thong-tin-co-dong/',
        ] 
    ...
    async def parse(self, response):
        ...
        for box in response.css('.news_column .panel-default'):
            pdf_link = box.css('a::attr(href)').get()
            date_str = box.css('li:has(.fa-clock-o)::text').get()
            title_raw = box.css('h2 a::text, h3 a::text').get(default='').strip()
