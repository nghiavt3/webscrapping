...
class EventSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [
            ('https://m.vtk.com.vn/pages/company/shareholder', self.parse_generic),
    ...
    def parse_generic(self, response):
        ...
        items = response.css('ion-item')
        
        for item in items:         
            title = item.css('a.title::text').get(default='').strip()
            # Kết hợp ngày và tháng năm thành một chuỗi ngày đầy đủ
            day = item.css('.day::text').get()
            month_year = item.css('.month_year::text').get()
            full_date = f"{day}/{month_year}" if day and month_year else None
