...
class EventSpider(scrapy.Spider):
    ...
    def parse(self, response):
        # Lặp qua tất cả các item trong tất cả các tab (tab1, tab2, tab3...)
        for report in response.css('div.item'):

            # 2. Trích xuất Tiêu đề, URL và Ngày đăng cho mỗi mục
            title = report.css('h4 a::text').get().strip()
            url = report.css('h4 a::attr(href)').get(),
            # Trích xuất Ngày đăng và làm sạch chuỗi
            date_text = report.css('p.archive-report-date::text').get().strip()
