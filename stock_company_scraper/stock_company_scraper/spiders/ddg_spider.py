...
class EventSpider(scrapy.Spider):
    name = 'event_ddg'
    mcpcty = 'DDG' 
    allowed_domains = ['dongduongcorp.com.vn'] 
    ...
    def start_requests(self):
        urls = [
            ('https://dongduongcorp.com.vn/vi/quan-he-co-dong', self.parse_generic),
    ...
    def parse_generic(self, response):
        ...
        for item in response.css('div.list ul li'):        
            title = item.css('a::attr(title)').get()
            date = item.css('div:not(.flex-1)::text').get()
