...
class EventSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [
            ('https://www.ptsc.com.vn/tin-co-dong', self.parse_generic),
    ...
    def parse_generic(self, response):
        ...
        articles = response.css('.news-card-item')

        for art in articles:
            title = art.css('.title a::text').get()
            relative_url = art.css('.title a::attr(href)').get()
            date_str = art.css('.time time::text').get()
