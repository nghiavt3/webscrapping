...
class EventSpider(scrapy.Spider):
    name = 'event_fcm'
    mcpcty = 'FCM' 
    allowed_domains = ['feconmining.com.vn'] 
    ...
    async def start(self):
        urls = [
            ('https://feconmining.com.vn/vi/quan-he-co-dong.html', self.parse_generic),
    ...
    def parse_generic(self, response):
        ...
        items = response.css('div.zoneteaser-i ul li[style*="margin-bottom:20px"]')
        
        for item in items:
            raw_date_str = item.css('h5 span::text').get()
            date_part = raw_date_str.split('-')[-1].strip()
