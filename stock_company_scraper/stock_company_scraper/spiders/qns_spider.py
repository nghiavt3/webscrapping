...
class EventSpider(scrapy.Spider):
    ...
    async def start(self):
        urls = [
            ('https://qns.com.vn/quan-he-co-dong', self.parse_generic), 
    ...
    async def parse_generic(self, response):
        ...
        items = response.css('.yearc1_item')
        
        for item in items:     
            title = item.css('.yearc1_name a::text').get()
            date = item.css('.yearc1_time::text').get()
            link = item.css('.yearc1_name a::attr(href)').get()
            if not title:
                continue
