...
class EventSpider(scrapy.Spider):
    ...
    async def start(self):
        urls = [
            ('https://www.gilimex.com/vi/danh-muc/cong-bo-thong-tin/', self.parse_generic),
    ...
    async def parse_generic(self, response):
        ...
        items = response.css('div.vii-qhcd-item')
        
        for item in items:           
            title = item.css('h3.vii-qhcd-item__title::text').get()
            date = item.css('time.vii-qhcd-item__time::text').get()
            link = item.css('a.vii-qhcd-item__inner::attr(href)').get()
            if not title:
                continue
