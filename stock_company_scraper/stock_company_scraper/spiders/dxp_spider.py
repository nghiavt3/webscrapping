...
class EventSpider(scrapy.Spider):
    name = 'event_dxp'
    mcpcty = 'DXP' 
    allowed_domains = ['doanxaport.com.vn'] 
    ...
    async def start(self):
        urls = [
            ('https://doanxaport.com.vn/quan-he-co-dong', self.parse_generic),
    ...
    def parse_generic(self, response):
        ...
        rows = response.css('.list-content-relations table tbody tr')
        
        for row in rows:     
            title = row.css('td:nth-child(1)::text').get()
            date_post_list = row.css('td:nth-child(2) span *::text').getall()
            date = "".join(date_post_list).strip()
            link = row.css('td:nth-child(3) a::attr(href)').get()
            pdf_link = row.css('td:nth-child(4) a::attr(href)').get()
            if not title:
                continue
