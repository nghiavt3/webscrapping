...
class EventSpider(scrapy.Spider):
    name = 'event_tbc'
    mcpcty = 'TBC'
    allowed_domains = ['thacba.vn'] 
    start_urls = [
        'https://thacba.vn/c2/pages-c/Quan-he-Co-dong-8.aspx',
        ] 
    ...
    async def parse(self, response):
        ...
        ''')
        posts = response.css('.blog-medium')
        for post in posts:
            pdf_link = post.css('h2 a::attr(href)').get()
            date_str = post.css('.blog-info li::text').get()
            title_raw = post.css('h2 a::text').get()
