import scrapy
import sqlite3
from stock_company_scraper.items import EventItem
from datetime import datetime
import re
from scrapy_playwright.page import PageMethod
class EventSpider(scrapy.Spider):
    name = 'event_pgb'
    mcpcty = 'PGB'
    allowed_domains = ['pgbank.com.vn'] 

    def __init__(self, *args, **kwargs):
        super(EventSpider, self).__init__(*args, **kwargs)
        self.db_path = 'stock_events.db'
    
    def start_requests(self):
        year = datetime.now().year
        urls = [
            ("https://www.pgbank.com.vn/nha-dau-tu/cong-bo", self.parse_generic),
            
        ]
        for url, callback in urls:
            yield scrapy.Request(
            url,
            meta={
                "playwright": True,
                "playwright_page_methods": [
                    # 1. Đợi mạng không còn tải thêm file nào (giúp tránh 403 nửa chừng)
                    PageMethod("wait_until", "networkidle"),
                    # 2. Đợi thêm một khoảng thời gian ngắn cho chắc chắn
                    PageMethod("wait_for_timeout", 2000),
                    # 3. Đợi bảng xuất hiện
                    PageMethod("wait_for_selector", "table.table-info tbody tr"),
                ],
            },
            callback=self.parse
        )

    def parse_generic(self, response):
        """Hàm parse dùng chung cho các chuyên mục"""
        # 1. Khởi tạo SQLite
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        table_name = f"{self.name}"
        #cursor.execute(f'''DROP TABLE IF EXISTS {table_name}''')
        cursor.execute(f'''
            CREATE TABLE IF NOT EXISTS {table_name} (
                id TEXT PRIMARY KEY, mcp TEXT, date TEXT, summary TEXT, 
                scraped_at TEXT, web_source TEXT, details_clean TEXT
            )
        ''')

        # Lấy tất cả các khối bao quanh bản tin
        # Lấy danh sách các item
        # Chọn tất cả các hàng trong thân bảng
        rows = response.css('table.table tbody tr')

        for row in rows:
            date = row.css('td:nth-child(3)::text').get()
            title = row.css('td:nth-child(2)::text').get()
            file_url = row.css('td:nth-child(4) span::text').get()
            absolute_url = response.urljoin(file_url)
            summary = title
            iso_date = convert_date_to_iso8601(date)

            # -------------------------------------------------------
            # 3. KIỂM TRA ĐIỂM DỪNG (INCREMENTAL LOGIC)
            # -------------------------------------------------------
            if iso_date :
                event_id = f"{summary}_{iso_date}".replace(' ', '_').strip()[:150]
            else :
                event_id = f"{summary}_NODATE".replace(' ', '_').strip()[:150]
            
            cursor.execute(f"SELECT id FROM {table_name} WHERE id = ?", (event_id,))
            if cursor.fetchone():
                self.logger.info(f"===> GẶP TIN CŨ: [{summary}]. DỪNG QUÉT CHUYÊN MỤC.")
                break 

            # 4. Yield Item
            e_item = EventItem()
            e_item['mcp'] = self.mcpcty
            e_item['web_source'] = self.allowed_domains[0]
            e_item['summary'] = summary
            e_item['date'] = iso_date
            e_item['details_raw'] = f"{summary}\nLink: {absolute_url}"
            e_item['scraped_at'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            
            yield e_item

        conn.close()

def convert_date_to_iso8601(vietnam_date_str):
    if not vietnam_date_str:
        return None
    try:
        date_object = datetime.strptime(vietnam_date_str.strip(), '%d/%m/%Y')
        return date_object.strftime('%Y-%m-%d')
    except ValueError:
        return None