...
class EventSpider(scrapy.Spider):
    ...
    async def start(self):
        urls = [
            ('https://haiants.vn/quan-he-co-dong.html', self.parse_generic),
    ...
    async def parse_generic(self, response):
        ...
        rows = response.css('div.list_file table tr:not(:first-child)')
        
        for row in rows:            
            title = row.css('td:nth-child(2)::text').get()
            date = row.css('td:nth-child(4)::text').get()
            link = row.css('td:nth-child(6) a::attr(href)').get()
            description= row.css('td:nth-child(5) *::text').getall()
            if not title:
                continue
