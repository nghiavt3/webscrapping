...
class EventSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [
            ('http://www.thaiduongpetrol.vn/dai-hoi-dong-co-dong', self.parse_generic),
            ('http://www.thaiduongpetrol.vn/cong-bo-thong-tin', self.parse_generic),
             ('http://www.thaiduongpetrol.vn/bao-cao-tai-chinh', self.parse_generic),
    ...
    def parse_generic(self, response):
        ...
        for article in articles:
            title = article.css('div.inner-image a::attr(title)').get()
            relative_url = article.css('h4.article-title a::attr(href)').get()
            date_str = article.css('span.post-date::text').get()
