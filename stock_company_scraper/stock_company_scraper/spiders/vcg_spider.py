...
class EventSpider(scrapy.Spider):
    ...
    async def start(self):
        urls = [
            ('https://vinaconex.com.vn/quan-he-co-dong/thong-tin-chung?page=1', self.parse), 
            ('https://vinaconex.com.vn/quan-he-co-dong/bao-cao-tai-chinh', self.parse_bctc), 
        ]
    ...
    async def parse_bctc(self, response):
        ...
        for report in response.css('.content-report'):
            title= report.css('.content-report-title p::text').get(default='').strip()
                
                # Lấy thuộc tính href của link tải
            download_url= report.css('.content-report-title a::attr(href)').get()
                
                # Lấy ngày tháng (thẻ p có class mr-10)
            date= report.css('.content-report-file p.mr-10::text').get(default='').strip()

            iso_date = convert_date_to_iso8601(date)

            summary = title
            full_url = response.urljoin(download_url) if download_url else "N/A"
            # -------------------------------------------------------
            # 3. KIỂM TRA ĐIỂM DỪNG (INCREMENTAL LOGIC)
            # -------------------------------------------------------
