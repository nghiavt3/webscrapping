...
class EventSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [
            ('http://bimico.vn/VanBan/Nhom_VanBan/14', self.parse_generic),
             ('http://bimico.vn/VanBan/Nhom_VanBan/15', self.parse_generic),
             ('http://bimico.vn/VanBan/Nhom_VanBan/16', self.parse_generic),
    ...
    def parse_generic(self, response):
        ...
        rows = response.css('table.vanban_phapluat tbody tr')
        
        for row in rows:          
            # Kiểm tra nếu hàng này chứa tiêu đề (thường có class 'table_tieude') thì bỏ qua
            if row.css('td.table_tieude'):
                continue  
            title = row.css('td:nth-child(2) a::text').get(default='').strip()
            date = row.css('td:nth-child(4) a::text').get(default='').strip()
