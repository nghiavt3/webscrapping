...
class EventSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for item in response.css('.item_ndt'):
            ...
            # 2. Trích xuất ngày tháng (giữ nguyên hoặc xử lý bỏ ngoặc)
            raw_date = item.css('.title a span.time::text').get()
            clean_date = raw_date.strip('()') if raw_date else None
            
            # 3. Trích xuất tiêu đề (Lấy text trực tiếp của thẻ <a>, bỏ qua text trong <span>)
            # Cách dùng ::text lồng nhau để lấy phần text không nằm trong span
