...
class EventSpider(scrapy.Spider):
    ...
    def parse(self, response):
        # Duyệt qua danh sách các bài viết (article)
        articles = response.css('article.elementor-post')
        
        for article in articles:
            title= article.css('.elementor-post__title a::text').get().strip(),
            link= article.css('.elementor-post__title a::attr(href)').get(),
            date= article.css('.elementor-post-date::text').get().strip(),
            time= article.css('.elementor-post-time::text').get().strip(),
