...
class EventSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [
            ('https://mstgroup.vn/co-dong/', self.parse_generic),
    ...
    def parse_generic(self, response):
        ...
        for doc in response.css('.doc-item'):        
            title = doc.css('.title::text').get()
            date = doc.css('.date-published::text').get()
            link = doc.css('a.btn-download::attr(href)').get()
            if not title:
                continue
