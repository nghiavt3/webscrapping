...
class EventSpider(scrapy.Spider):
    name = 'event_dnc'
    mcpcty = 'DNC' 
    allowed_domains = ['diennuochp.com.vn'] 
    ...
    async def start(self):
        urls = [
            ('https://diennuochp.com.vn/TinTuc?categories=BCTC', self.parse_generic),
            ('https://diennuochp.com.vn/TinTuc?categories=CBTT', self.parse_generic),
    ...
    async def parse_generic(self, response):
        ...
        ''')
        # Lặp qua từng khối bài viết
        articles = response.css('div.article')
        
        for article in articles:          
            title = article.css('h4.new__title b::text').get()
            date_raw = article.css('div.time p::text').get()
            date = date_raw.replace('Ngày:', '').strip() if date_raw else None
