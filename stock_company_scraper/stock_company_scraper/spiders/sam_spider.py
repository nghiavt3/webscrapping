...
class EventSpider(scrapy.Spider):
    name = 'event_sam'
    mcpcty = 'SAM' 
    allowed_domains = ['samholdings.com.vn'] 
    ...
    def start_requests(self):
        urls = [
            ('https://samholdings.com.vn/documents/dai-hoi-dong-co-dong/', self.parse_generic),
            ('https://samholdings.com.vn/documents/thong-tin-bat-thuong-va-theo-yeu-cau/', self.parse_generic),
            ('https://samholdings.com.vn/documents/thong-tin-dinh-ky/', self.parse_generic),
    ...
    def parse_generic(self, response):
       ...
       # cursor.execute(f'''DROP TABLE IF EXISTS {table_name}''')
        ...
        items = response.css('ul.list li')
        for item in items:    

            title = item.css('h4::text').get()
            raw_time = item.css('div::text').get()
            date_part = raw_time.split('|')[0].strip()
            link = item.css('a[download]::attr(href)').get()
            if not title:
                continue

            summary = title.strip()
            iso_date = clean_date_to_iso(date_part)
