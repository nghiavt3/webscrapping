...
class EventSpider(scrapy.Spider):
    ...
    async def start(self):
        urls = [
            ('https://www.ctin.vn/thong-bao/', self.parse_generic),
            ('https://www.ctin.vn/dai-hoi-co-dong-thuong-nien/', self.parse_generic),
            ('https://www.ctin.vn/bao-cao-tai-chinh/', self.parse_generic),
    ...
    async def parse_generic(self, response):
        ...
        for item in response.css('.info-box-wrapper'):
            date_raw = item.css('.info-box-icon::text').get()
            date = date_raw.strip() if date_raw else None
            title = item.css('.info-box-title::text').get()
            
            link = item.css('.info-box-inner a::attr(href)').get()
            if not title:
                continue
