...
class EventSpider(scrapy.Spider):
    name = 'event_gmd'
    mcpcty = 'GMD' 
    allowed_domains = ['gemadept.com.vn'] 
    ...
    def start_requests(self):
        urls = [
            ('https://www.gemadept.com.vn/co-dong/thong-bao/', self.parse_generic),
            ('https://www.gemadept.com.vn/co-dong/bao-cao-tai-chinh/', self.parse_generic),
    ...
    def parse_generic(self, response):
        ...
        items = response.css('div.list-info-notify div.content')
        
        for item in items:        
            title = item.css('h5.body-1::text').get()
            date = item.css('div.date::text').get()
            link = item.css('div.wrap-title a::attr(href)').get()
