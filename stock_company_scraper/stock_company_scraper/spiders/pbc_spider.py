...
class EventSpider(scrapy.Spider):
    ...
    async def start(self):
        urls = [
            ('https://cpc1.com.vn/co-dong/cat11/Cong-Bo-Thong-Tin', self.parse_generic),
             ('https://cpc1.com.vn/co-dong/cat1/Dai-Hoi-Co-Dong', self.parse_generic),
              ('https://cpc1.com.vn/co-dong/cat12/Bao-Cao-Tai-Chinh', self.parse_generic),
    ...
    async def parse_generic(self, response):
        ...
        for item in response.css('div.list_sh ul li'):       
            title = item.css('div.title a::text').get()
            date = None
            link = item.css('div.title a::attr(href)').get()
            if not title:
                continue
