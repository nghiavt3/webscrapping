...
class EventSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        # 2. Lấy danh sách records (bỏ qua dòng đầu tiên nếu là header)
        records = response.css('.itemList div.MTContainer:not(:first-child)')
        
        for record in records:
            title_raw = record.css('h2.MTItemTitle a::text').get()
            # Sử dụng urljoin để an toàn hơn thay vì cộng chuỗi thủ công
            doc_url = response.urljoin(record.css('h2.MTItemTitle a::attr(href)').get())
            download_url = response.urljoin(record.css('div.MTItemDownload a::attr(href)').get())
            ...
            iso_date = convert_date_to_iso8601(date_created)
