...
class EventSpider(scrapy.Spider):
    ...
    async def parse_generic(self, response):
        ...
        posts = response.css('div.post-item-wrapper')

        for post in posts:
            title = post.css('.title a::text').get()
            relative_url = post.css('.title a::attr(href)').get()
            raw_date = post.css('span.published-date::text').get()
