...
class EventSpider(scrapy.Spider):
    ...
    def parse_generic(self, response):
        ...
        rows = response.css('tr.document-item')

        for row in rows:
            title = row.css('p.document-title::text').get().strip()
            relative_url = row.css('td:nth-child(4) a::attr(href)').get()
            date_str = row.css('td:nth-child(2)::text').get().strip()
