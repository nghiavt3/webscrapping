...
class EventSpider(scrapy.Spider):
    name = 'event_sp2'
    mcpcty = 'SP2'
    allowed_domains = ['supan2.net'] 
    start_urls = [
        'http://supan2.net/category/quan-he-co-dong/',
        ] 
    ...
    async def parse(self, response):
        ...
        ''')
        for post in response.css('.news-post'):
            pdf_link = post.css('h2 a::attr(href)').get()
            date_str = post.css('div span::text').get()
            title_raw = post.css('h2 a::text').get(default='').strip()
